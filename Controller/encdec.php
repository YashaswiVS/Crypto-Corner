...
// You should use "r" for opening text files.

// You should use "rb" if you're opening non-text files.
 
class AES_CBC
{
   protected static $KEY_SIZES = array('AES-128'=>10,'AES-192'=>12,'AES-256'=>14);
   ...
   public static function encryptFile($key, $input_stream, $aes_filename){
      ...
      if (!empty($fin) && !empty($fc)) {
         ...
         $ekey = hash_pbkdf2("sha256", $key, $esalt, $it=1000, self::key_size(), $raw=true);
         ...
         fclose($fin);
         fclose($fc);
         $hmac_raw = self::calculate_hmac_after_32bytes($key, $hmac_salt, $aes_filename);
   ...
   public static function decryptFile($key, $aes_filename, $out_stream) {
      ...
      $hmac_calc = self::calculate_hmac_after_32bytes($key, $hmac_salt, $aes_filename);
   ...
   private static function calculate_hmac_after_32bytes($key, $hsalt, $filename) {
      static $init=0;
      $init or $init = stream_filter_register("user-filter.skipfirst32bytes", "FileSkip32Bytes");

   
      $stream = 'php://filter/read=user-filter.skipfirst32bytes/resource=' . $filename;
      $hkey = hash_pbkdf2("sha256", $key, $hsalt, $iterations=1000, 24, $raw=true);
