		...
		<div class="main">
			...
			<div class="main-content">
				...
				<thead class="thead-dark">
					<tr>
					<th scope="col">#</th>
					<th width="200" scope="col">Client Name</th>
					<th scope="col">Client Office </th>
					<th scope="col">File Name</th>
					<th scope="col">File Category</th>
					<th width="100" scope="col" class="text-center">Action</th>
					</tr>
				</thead>
				<tbody>
				...
				<?php
					...
					{ 
						...
						$bstmt = $admin -> ret("SELECT * FROM `files` WHERE `cl_id` = '$cid'");
						while($brow = $bstmt-> fetch(PDO::FETCH_ASSOC)) { 
						$fcid = $brow['c_id'];

						$astmt = $admin -> ret("SELECT * FROM `clients` WHERE `cl_id` = '$cid'");
						$arow = $astmt-> fetch(PDO::FETCH_ASSOC);
						$abstmt = $admin -> ret("SELECT * FROM `file_category` WHERE `c_id` = '$fcid'");
				...
				<tr>
					...
					<td><?php echo $brow['fileName']; ?></td>
					<td><?php echo $abrow['cat_name']; ?></td>
					<td>
					<a href="../../Controller/download.php?id=<?php echo $brow['f_id']; ?>&from=empdfiles"><button name="dlt" class="btn btn-success">DOWNLOAD (<?= $brow['file_size']?>)</button></a>
